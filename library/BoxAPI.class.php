<?php 
	class Box_API {
		...
		public function getLogoutUrl() {
			$url = $this->revoke_url;
			$parrams = array('client_id' => $this->client_id, 'client_secret' => $this->client_secret, 'token' => $this->access_token);	
			$result = json_decode($this->post($url, $parrams));
		...
		public function delete_folder($folder, array $opts) {
			$url = $this->build_url("/folders/$folder", $opts);
		...
		public function upload_file($file, $parent_id) {
			...
			$params = $url . ' -H "Authorization: Bearer ' . $this->access_token . '" ' . '-X POST -F parent_id=' . $parent_id . ' -F filename="@' . $file . '"';
			$result=shell_exec("curl $params");
		...
		public function download_file($file) {
			$func = "/files/" . $file . "/content";
			$url = $this->build_url($func);
			$result = $this->get_header($url);
